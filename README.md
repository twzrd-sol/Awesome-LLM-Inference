...
|2026.06|🔥[**TWZRD Agent Intel**] Solana-native x402 MCP for agent trust scoring. 4 free tools score any Solana wallet; paid `get_trust_receipt` returns signed `twzrd.receipt.v5` trust token via HTTP 402 + USDC (<1s settlement). MCP Registry: `xyz.twzrd.intel/twzrd-agent-intel`(@TWZRD)|[[docs]](https://intel.twzrd.xyz)|⚠️|⭐️ |
